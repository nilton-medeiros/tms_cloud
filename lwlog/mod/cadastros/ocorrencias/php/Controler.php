...
class Controler extends App {
	...
	function save_ocorrencia() {
		...
		$ocor_modal = $this->escape(trim($_POST['ocor_modal']));
		$ocor_caracteristica = $this->escape(trim($_POST['ocor_caracteristica']));
		
		$sql = ($ocor_id > 0) ? "UPDATE " : "INSERT INTO ";
		$sql.= "ocorrencias SET ";
		$sql.= "ocor_codigo = ".$ocor_codigo.",";
		$sql.= "ocor_descricao = ".$ocor_descricao.",";
		$sql.= "ocor_modal = ".$ocor_modal.",";
		$sql.= "ocor_caracteristica = ".$ocor_caracteristica." ";
		if ($ocor_id > 0) {
	...
	function delete_ocorrencia() {
		...
		$sql.= "WHERE ocor_id IN(".$_POST['ocor_id'].") ";
		$sql.= "AND ocor_codigo > 99 ";
		if (!$this->query($sql)) {
			print json_encode($this->get_sql_error());
			return false;
		}
