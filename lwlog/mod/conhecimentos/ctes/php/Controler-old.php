		...
		if ($cte_id > 0) {
			...
			while ($field = $this->fetch_object($query)) {
				...
				if ($field->cf_tipo == 2) {
					...
					$exist_taxa = false;
					if ($tomador->clie_id > 0) {
						$s = "SELECT COUNT(*) AS existente FROM taxas WHERE emp_id = ".$this->empresa->emp_id." AND clie_id = ".$tomador->clie_id;
						$this->debug($s);
						$q = $this->query($s);
						$exist_taxa = $this->fetch_object($q)->existente > 0;
						$this->free_result($q);
					}
					$s = "SELECT ";
					$s.= "t2.cc_id,";
					...
					if ($exist_taxa) {
						$s.= "AND t1.clie_id = ".$tomador->clie_id." ";
						$s.= "AND t1.clie_id NOT IN (SELECT t3.clie_id FROM taxas_excecoes_clientes AS t3 WHERE t3.tx_id = t1.tx_id) ";
					} elseif ($tomador->clie_id > 0) {
						$s.= "AND t1.clie_id IS NULL ";
						$s.= "AND t1.tx_id NOT IN (SELECT t3.tx_id FROM taxas_excecoes_clientes AS t3 WHERE t3.clie_id = ".$tomador->clie_id.") ";
					} else {
						$s.= "AND t1.clie_id IS NULL ";
						$s.= "AND t1.tx_id NOT IN (SELECT t3.tx_id FROM taxas_excecoes_clientes AS t3 WHERE t3.clie_id IS NULL) ";
					}
					$s.= "AND t2.cf_id = ".$field->cf_id." ";
					$s.= "AND t1.cid_id = ".$cidade_destino->cid_id;
					$this->debug($s);
					$q = $this->query($s);
