	...
	*/
	class TCPDF {
		...
		public function setPageUnit($unit) {
			...
			switch ($unit) {
				...
				// unsupported unit
				default : {
					$this->ExtJSException('Incorrect unit: '.$unit);
					break;
				}
			}
			$this->pdfunit = $unit;
		...
		public function SetDisplayMode($zoom, $layout='SinglePage', $mode='UseNone') {
			...
			if (($zoom == 'fullpage') OR ($zoom == 'fullwidth') OR ($zoom == 'real') OR ($zoom == 'default') OR (!is_string($zoom))) {
				...
				$this->ExtJSException('Incorrect zoom display mode: '.$zoom);
			}
			switch ($layout) {
				case 'default':
				case 'single':
		 ...
		 * @since 1.0
		 */
		public function ExtJSException($msg) {
			// unset all class variables
			$this->_destroy(true);
		...
		public function setPage($pnum, $resetmargins=false) {
			...
			if (($pnum > 0) AND ($pnum <= $this->numpages)) {
				...
				$this->ExtJSException('Wrong page number on setPage() function.');
			}
		}
		...
		public function SetDrawSpotColor($name, $tint=100) {
			if (!isset($this->spot_colors[$name])) {
				$this->ExtJSException('Undefined spot color: '.$name);
			}
		...
		public function AddFont($family, $style='', $fontfile='') {
			if ($this->empty_string($family)) {
				if (!$this->empty_string($this->FontFamily)) {
					...
					$this->ExtJSException('Empty font family');
				}
			}
			...
			if (file_exists($fontfile)) {
				...
				$this->ExtJSException('Could not include font definition file: '.$family.'');
			}
			// check font parameters
			if ((!isset($type)) OR (!isset($cw))) {
				$this->ExtJSException('The font definition file has a bad format: '.$fontfile.'');
			}
			...
			if ($type == 'cidfont0') {
				...
				$this->ExtJSException('Unknow font type: '.$type.'');
			}
			...
			if ($imsize === FALSE) {
				...
				if ($imsize === FALSE) {
					if (($w > 0) AND ($h > 0)) {
						...
						$this->ExtJSException('[Image] Unable to get image width and height: '.$file);
					}
				}
			}
		...
		protected function _parsejpeg($file) {
			$a = getimagesize($file);
			if (empty($a)) {
				$this->ExtJSException('Missing or incorrect image file: '.$file);
			}
			if ($a[2] != 2) {
				$this->ExtJSException('Not a JPEG file: '.$file);
			}
		...
		protected function _parsepng($file) {
			$f = fopen($file, 'rb');
			if ($f === false) {
				$this->ExtJSException('Can\'t open image file: '.$file);
			}
			//Check signature
			if (fread($f, 8) != chr(137).'PNG'.chr(13).chr(10).chr(26).chr(10)) {
				$this->ExtJSException('Not a PNG file: '.$file);
			}
			//Read header chunk
			fread($f, 4);
			if (fread($f, 4) != 'IHDR') {
				$this->ExtJSException('Incorrect PNG file: '.$file);
			}
			$w = $this->_freadint($f);
			$h = $this->_freadint($f);
			$bpc = ord(fread($f, 1));
			if ($bpc > 8) {
				//$this->ExtJSException('16-bit depth not supported: '.$file);
				fclose($f);
				return false;
			}
			$ct = ord(fread($f, 1));
			...
			if (ord(fread($f, 1)) != 0) {
				//$this->ExtJSException('Unknown compression method: '.$file);
				fclose($f);
				return false;
			}
			if (ord(fread($f, 1)) != 0) {
				//$this->ExtJSException('Unknown filter method: '.$file);
				fclose($f);
				return false;
			}
			if (ord(fread($f, 1)) != 0) {
				//$this->ExtJSException('Interlacing not supported: '.$file);
				fclose($f);
				return false;
			}
			fread($f, 4);
			...
			if (($colspace == 'Indexed') AND (empty($pal))) {
				//$this->ExtJSException('Missing palette in '.$file);
				fclose($f);
				return false;
			}
			fclose($f);
		...
		public function Output($name='doc.pdf', $dest='I') {
			...
			if ($this->sign) {
				...
				$f = fopen($tempdoc, 'wb');
				if (!$f) {
					$this->ExtJSException('Unable to create temporary file: '.$tempdoc);
				}
			...
			switch($dest) {
				case 'I': {
					...
					if (ob_get_contents()) {
						$this->ExtJSException('Some data has already been output, can\'t send PDF file');
					}
					if (php_sapi_name() != 'cli') {
						//We send to a browser
						header('Content-Type: application/pdf');
						if (headers_sent()) {
							$this->ExtJSException('Some data has already been output to browser, can\'t send PDF file');
						}
				...
				case 'FD': {
					...
					if ($this->diskcache) {
						...
						$f = fopen($name, 'wb');
						if (!$f) {
							$this->ExtJSException('Unable to create output file: '.$name);
						}
				...
				default: {
					$this->ExtJSException('Incorrect output destination: '.$dest);
				}
			}
			return '';
		}
		...
		protected function _dochecks() {
			//Check for locale-related bug
			if (1.1 == 1) {
				$this->ExtJSException('Don\'t alter the locale before including class file');
			}
			...
			foreach ($this->fontkeys as $k) {
				...
				if ($type == 'core') {
					...
					if (!method_exists($this, $mtd)) {
						$this->ExtJSException('Unsupported font type: '.$type);
					}
		...
		protected function _puttruetypeunicode($font) {
			...
			if (isset($font['ctg']) AND (!$this->empty_string($font['ctg']))) {
				...
				if ($this->empty_string($fontfile)) {
					$this->ExtJSException('Font file not found: '.$ctgfile);
				}
		...
		public function SetProtection($permissions=array(), $user_pass='', $owner_pass=null) {
			...
			foreach ($permissions as $permission) {
				if (!isset($options[$permission])) {
					$this->ExtJSException('Incorrect permission: '.$permission);
				}
		...
		public function Scale($s_x, $s_y, $x='', $y='') {
			...
			if (($s_x == 0) OR ($s_y == 0)) {
				$this->ExtJSException('Please do not use values equal to zero for scaling');
			}
			$y = ($this->h - $y) * $this->k;
			$x *= $this->k;
		...
		public function Skew($angle_x, $angle_y, $x='', $y='') {
			...
			if (($angle_x <= -90) OR ($angle_x >= 90) OR ($angle_y <= -90) OR ($angle_y >= 90)) {
				$this->ExtJSException('Please use values between -90 and +90 degrees for Skewing.');
			}
			$x *= $this->k;
			$y = ($this->h - $y) * $this->k;
			//calculate elements of transformation matrix
		...
		protected function _JScolor($color) {
			...
			if (!in_array($color,$aColors)) {
				$this->ExtJSException('Invalid color: '.$color);
			}
			return 'color.'.$color;
		}
		...
		public function setVisibility($v) {
			...
			switch($v) {
				...
				default: {
					$this->ExtJSException('Incorrect visibility: '.$v);
					break;
				}
			}
			$this->visibility = $v;
		...
		public function ImageEps($file, $x='', $y='', $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false) {
			...
			$data = file_get_contents($file);
			if ($data === false) {
				$this->ExtJSException('EPS file not found: '.$file);
			}
			$regs = array();
			...
			if (count($regs) > 1) {
				...
				if (strpos($version_str, 'Adobe Illustrator') !== false) {
					...
					if ($version >= 9) {
						$this->ExtJSException('This version of Adobe Illustrator file is not supported: '.$file);
					}
				}
			}
			...
			if (count($regs) > 1) {
				...
				$this->ExtJSException('No BoundingBox found in EPS file: '.$file);
			}
		...
		public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres=0.4, $style='', $align='') {
			...
			if ($arrcode === false) {
				$this->ExtJSException('ExtJSException in 1D barcode string');
			}
			// set default values
		...
		public function write2DBarcode($code, $type, $x='', $y='', $w='', $h='', $style='', $align='') {
			...
			$arrcode = $barcodeobj->getBarcodeArray();
			if ($arrcode === false) {
				$this->ExtJSException('ExtJSException in 2D barcode string');
			}
			// set default values
			if (!isset($style['fgcolor'])) {
				$style['fgcolor'] = array(0,0,0); // default black
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
								...
								if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
									...
									while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
										...
										if (($stroffset !== false) AND ($stroffset <= $strpiece[2][1])) {
											...
											if ($offset === false) {
												$this->ExtJSException('HTML Justification: malformed PDF code.');
											}
											continue;
										}
										if ($this->isRTLTextDir()) {
		...
		protected function writeDiskCache($filename, $data, $append=false) {
			...
			if (!$f) {
				$this->ExtJSException('Unable to write cache file: '.$filename);
			} else {
				fwrite($f, $data);
				fclose($f);
			}
		...
		public function ImageSVG($file, $x='', $y='', $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false) {
			...
			if ($svgdata === false) {
				$this->ExtJSException('SVG file not found: '.$file);
			}
			if ($x === '') {
				$x = $this->x;
			}
			...
			if(!xml_parse($this->parser, $svgdata)) {
				$error_message = sprintf("SVG ExtJSException: %s at line %d", xml_error_string(xml_get_error_code($this->parser)), xml_get_current_line_number($this->parser));
				$this->ExtJSException($error_message);
			}
