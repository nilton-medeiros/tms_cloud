...
function MakeFont($fontfile, $fmfile, $embedded=true, $enc='cp1252', $patch=array()) {
	...
	if (!file_exists($fontfile)) {
		die('ExtJSException: file not found: '.$fontfile);
	}
	if (!file_exists($fmfile)) {
		die('ExtJSException: file not found: '.$fmfile);
	}
	$cidtogidmap = '';
	$map = array();
	$diff = '';
	...
	if ($fmext == 'afm') {
		if (($ffext == 'ttf') OR ($ffext == 'otf')) {
			...
			die('ExtJSException: unrecognized font file extension: '.$ffext);
		}
		if ($enc) {
			$map = ReadMap($enc);
		...
		if (($ffext == 'ttf') OR ($ffext == 'otf')) {
			...
			die('ExtJSException: not a TrueType font: '.$ffext);
		}
	...
	if ($embedded) {
		...
		$f = fopen($fontfile,'rb');
		if (!$f) {
			die('ExtJSException: Unable to open '.$fontfile);
		}
		...
		if ($type == 'Type1') {
			...
			if (!$pos) {
				die('ExtJSException: font file does not seem to be valid Type1');
			}
			$size1 = $pos + 6;
...
function ReadMap($enc) {
	...
	$a = file($file);
	if (empty($a)) {
		die('ExtJSException: encoding not found: '.$enc);
	}
	$cc2gn = array();
	foreach ($a as $l) {
		if ($l{0} == '!') {
...
function CheckTTF($file) {
	...
	$f = fopen($file, 'rb');
	if (!$f) {
		die('ExtJSException: unable to open '.$file);
	}
