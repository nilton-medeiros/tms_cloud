...
class parsingHTML
{
			...
			if ($part[0]=='code')
			{
				...
				if ($res)
				{
					...
					if (!in_array($res['name'], $balises_no_closed))
					{
						...
						{
							if (count($parents)<1)
								throw new HTML2PDF_exception(3, $res['name'], $this->getHtmlExtJSExceptionCode($res['html_pos']));
							else if ($parents[count($parents)-1]!=$res['name'])
								throw new HTML2PDF_exception(4, $parents, $this->getHtmlExtJSExceptionCode($res['html_pos']));
							else
	...
	public function getHtmlExtJSExceptionCode($pos)
	{
