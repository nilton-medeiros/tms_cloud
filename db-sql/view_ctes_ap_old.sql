-- Conferido- Baixado do DB tmscte_lw em 2022-11-04 as 12:10 - sincronizado novos campos do DIFAL+FCP
  ...
  (
    (
      (
                                                    ...
                                                    (
                                                      (
                                                        (
                                                          (
                                                            tmscte_ap.ctes t1
                                                            left join tmscte_ap.usuarios t2 on((t2.user_id = t1.cte_cadastrado_por))
                                                          )
                                                          left join tmscte_ap.usuarios t3 on((t3.user_id = t1.cte_alterado_por))
                                                        )
                                                        left join tmscte_ap.ctes_faturados t4 on((t4.cte_id = t1.cte_id))
                                                      )
                                                      left join tmscte_ap.clientes t5 on((t5.clie_id = t1.clie_remetente_id))
                                                    )
                                                    left join tmscte_ap.clientes t6 on((t6.clie_id = t1.clie_coleta_id))
                                                  )
                                                  left join tmscte_ap.clientes t7 on((t7.clie_id = t1.clie_expedidor_id))
                                                )
                                                left join tmscte_ap.clientes t8 on((t8.clie_id = t1.clie_recebedor_id))
                                              )
                                              left join tmscte_ap.clientes t9 on((t9.clie_id = t1.clie_destinatario_id))
                                            )
                                            left join tmscte_ap.clientes t10 on((t10.clie_id = t1.clie_entrega_id))
                                          )
                                          left join tmscte_ap.clientes t11 on((t11.clie_id = t1.clie_tomador_id))
                                        )
                                        left join tmscte_ap.clientes t22 on((t22.clie_id = t1.clie_representante_id))
                                      )
                                      left join tmscte_ap.cidades cid5 on((cid5.cid_id = t5.cid_id))
                                    )
                                    left join tmscte_ap.cidades cid6 on((cid6.cid_id = t6.cid_id))
                                  )
                                  left join tmscte_ap.cidades cid7 on((cid7.cid_id = t7.cid_id))
                                )
                                left join tmscte_ap.cidades cid8 on((cid8.cid_id = t8.cid_id))
                              )
                              left join tmscte_ap.cidades cid9 on((cid9.cid_id = t9.cid_id))
                            )
                            left join tmscte_ap.cidades cid10 on((cid10.cid_id = t10.cid_id))
                          )
                          left join tmscte_ap.cidades cid11 on((cid11.cid_id = t11.cid_id))
                        )
                        left join tmscte_ap.cidades t12 on((t12.cid_id = t1.cid_id_origem))
                      )
                      left join tmscte_ap.cidades t13 on((t13.cid_id = t1.cid_id_destino))
                    )
                    left join tmscte_ap.cidades t14 on((t14.cid_id = t1.cid_id_passagem))
                  )
                  left join tmscte_ap.cidades t19 on(
                    (t19.cid_id = t1.cid_id_etiqueta_entrega)
                  )
                )
                left join tmscte_ap.produtos t15 on((t15.prod_id = t1.prod_id))
              )
              left join tmscte_ap.grupo_tarifas t16 on((t16.gt_id_codigo = t15.gt_id_codigo))
            )
            left join tmscte_ap.iata_imp_codes t17 on((t17.iic_id = t15.iic_id))
          )
          left join tmscte_ap.ctes_eventos t18 on(
            (
        ...
        left join tmscte_ap.view_sum_entrega_ctes_ocorrencias t20 on((t20.cte_id = t1.cte_id))
      )
      left join tmscte_ap.view_ctes_documentos_group t21 on((t21.cte_id = t1.cte_id))
    )
    join tmscte_ap.empresas t23 on((t23.emp_id = t1.emp_id))
  )
group by
  t1.cte_id